...
Input modes:
  ...
  B) Online FinanceDataReader:
       --online
     Requires finance-datareader and pykrx and internet.
...
def fetch_current_kospi200_online():
    ...
    dates = pd.date_range(pd.Timestamp.today().normalize(), periods=10, freq="-1D")
    # KOSPI200 index ticker is usually 1028; resolve by name to avoid hardcoding.
    idxs = stock.get_index_ticker_list(market="KOSPI")
    code = next(c for c in idxs if stock.get_index_ticker_name(c) == "코스피 200")
    tickers = []
    for dt in dates:
        try:
            tickers = stock.get_index_portfolio_deposit_file(code, dt.strftime("%Y%m%d"))
            if tickers:
                break
        except Exception:
            pass
    if not tickers:
        raise RuntimeError("Could not fetch current KOSPI200 constituents.")
    return pd.DataFrame({
        "Code": tickers,
...
def _save_portfolio_equity_chart(portfolio: pd.DataFrame, outdir: Path):
    """Save equal-weight portfolio equity curve and drawdown."""
    if portfolio.empty:
        return
    ...
    ax.plot(portfolio.index, portfolio["Equity"], lw=1.8, label="CCI Strategy Equity")
    ax.axhline(1.0, linestyle="--", alpha=0.5)
    ax.set_title("CCI(9) + DMI(14) + PSAR Portfolio Equity")
    ax.set_xlabel("Date")
    ...
    fig.tight_layout()
    fig.savefig(outdir / "portfolio_equity.png", dpi=180, bbox_inches="tight")
    plt.close(fig)

    dd = portfolio["Equity"] / portfolio["Equity"].cummax() - 1.0
    fig = plt.figure(figsize=(14, 6))
    ax = fig.add_subplot(111)
    ax.plot(portfolio.index, dd, lw=1.4, label="Drawdown")
    ...
    fig.tight_layout()
    fig.savefig(outdir / "portfolio_drawdown.png", dpi=180, bbox_inches="tight")
    plt.close(fig)


def _save_return_comparison_chart(by_stock: pd.DataFrame, outdir: Path, top_n: int = 30):
    """
    Compare strategy vs buy-and-hold for the most active names.
    This keeps the image readable when the universe contains ~200 stocks.
    """
    if by_stock.empty:
        return

    plot_df = by_stock.copy()
    plot_df["Code"] = plot_df["Code"].astype(str).str.zfill(6)
    plot_df = plot_df.sort_values(["Trades", "StrategyReturn"], ascending=[False, False]).head(top_n)
    ...
    ax = fig.add_subplot(111)
    ax.bar(x - width / 2, plot_df["StrategyReturn"], width, label="Strategy")
    ax.bar(x + width / 2, plot_df["BuyHoldReturn"], width, label="Buy & Hold")
    ax.axhline(0, linewidth=0.8)
    ax.set_xticks(x)
    ax.set_xticklabels(plot_df["Code"], rotation=75, ha="right")
    ax.set_title(f"Strategy vs Buy & Hold — Top {len(plot_df)} by Trade Count")
    ax.set_ylabel("Return")
    ax.grid(True, axis="y", alpha=0.2)
    ax.legend()
    fig.tight_layout()
    fig.savefig(outdir / "strategy_vs_buyhold.png", dpi=180, bbox_inches="tight")
    plt.close(fig)


def _save_trade_distribution_chart(trades: pd.DataFrame, outdir: Path):
    if trades.empty or "Return" not in trades.columns:
        return

    r = trades["Return"].replace([np.inf, -np.inf], np.nan).dropna()
    if r.empty:
        return

    fig = plt.figure(figsize=(12, 7))
    ax = fig.add_subplot(111)
    bins = min(50, max(10, int(np.sqrt(len(r)))))
    ax.hist(r, bins=bins, alpha=0.8)
    ax.axvline(0, linestyle="--", linewidth=1.0)
    ax.axvline(r.mean(), linestyle=":", linewidth=1.2, label=f"Mean {r.mean():.2%}")
    ...
    ax.legend()
    fig.tight_layout()
    fig.savefig(outdir / "trade_return_distribution.png", dpi=180, bbox_inches="tight")
    plt.close(fig)


def _save_top_signal_charts(
    ...
    """
    Plot Close/PSAR plus actual entry/exit points for the most frequently traded stocks.
    Signal logic is identical to the backtest; this function only visualizes it.
    """
    if by_stock.empty or not raw_prices:
        return

    ranked = (
        by_stock.sort_values(["Trades", "StrategyReturn"], ascending=[False, False])
        .head(top_n)
        .copy()
    )
    ...
    for ax, (_, stat) in zip(axes, ranked.iterrows()):
        ax.set_visible(True)
        code = str(stat["Code"]).zfill(6)
        z = raw_prices.get(code)

        if z is None or z.empty:
            ax.set_title(f"{code} — no plot data")
            continue

        x = calc_indicators(z.sort_index().copy())
        x = x.loc[(x.index >= pd.Timestamp(start)) & (x.index <= pd.Timestamp(end))]
        if x.empty:
            ax.set_title(f"{code} — no evaluation data")
            continue

        ax.plot(x.index, x["Close"], lw=1.0, label="Close")
        ax.plot(x.index, x["PSAR"], linestyle=":", lw=0.9, alpha=0.8, label="PSAR")

        if not trades.empty:
            t = trades[trades["Code"].astype(str).str.zfill(6) == code]
            if not t.empty:
                e_dates = pd.to_datetime(t["EntryDate"])
                x_dates = pd.to_datetime(t["ExitDate"])

                # Entry/exit prices in the trade log are net execution prices.
                ax.scatter(
                    e_dates, t["EntryPriceNet"],
                    marker="^", s=70, edgecolors="black",
                    zorder=5, label="Entry"
                )
                ax.scatter(
                    x_dates, t["ExitPriceNet"],
                    marker="v", s=70, edgecolors="black",
                    zorder=5, label="Exit"
                )

        ax.set_title(
            f"{code} | Trades {int(stat['Trades'])} | "
            f"Ret {stat['StrategyReturn']:.1%} | Win {stat['WinRate']:.1%}"
            if pd.notna(stat["WinRate"])
            else f"{code} | Trades {int(stat['Trades'])} | Ret {stat['StrategyReturn']:.1%}"
        )
        ax.grid(True, alpha=0.2)
    ...
    fig.suptitle("Most Active CCI/DMI/PSAR Backtest Signals", fontsize=16)
    fig.tight_layout(rect=[0, 0, 1, 0.98])
    fig.savefig(outdir / "top_signal_charts.png", dpi=180, bbox_inches="tight")
    plt.close(fig)


def _save_top_tables(by_stock: pd.DataFrame, trades: pd.DataFrame, outdir: Path):
    """Extra CSVs that make GitHub artifacts easier to inspect."""
    if not by_stock.empty:
        by_stock.sort_values("StrategyReturn", ascending=False).head(30).to_csv(
            outdir / "top30_strategy_return.csv", index=False, encoding="utf-8-sig"
        )
        by_stock.sort_values("Trades", ascending=False).head(30).to_csv(
            outdir / "top30_trade_count.csv", index=False, encoding="utf-8-sig"
        )

    if not trades.empty:
        trades.sort_values("Return", ascending=False).head(50).to_csv(
            outdir / "best50_trades.csv", index=False, encoding="utf-8-sig"
        )
        trades.sort_values("Return", ascending=True).head(50).to_csv(
            outdir / "worst50_trades.csv", index=False, encoding="utf-8-sig"
        )
...
def main():
    ...
    for j, row in universe.iterrows():
        ...
        try:
            ...
            raw_prices[str(code).zfill(6)] = z.copy()
    ...
    # Visual reports and additional inspection tables.
    _save_portfolio_equity_chart(portfolio, outdir)
    _save_return_comparison_chart(by_stock, outdir, top_n=30)
    _save_trade_distribution_chart(trades, outdir)
    _save_top_signal_charts(
    ...
    _save_top_tables(by_stock, trades, outdir)

    print("\n=== SUMMARY ===")
    for k, v in summary.items():
        print(f"{k}: {v}")
    print(f"\nSaved to: {outdir.resolve()}")
    print("Generated visual reports:")
    print(" - portfolio_equity.png")
    print(" - portfolio_drawdown.png")
    print(" - strategy_vs_buyhold.png")
    print(" - trade_return_distribution.png")
    print(" - top_signal_charts.png")
